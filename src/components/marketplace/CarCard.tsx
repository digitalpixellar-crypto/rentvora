...
export default function CarCard({ car, rentalType = 'self_drive' }: { car: Car; rentalType?: 'self_drive' | 'with_driver' }) {
  ...
  const isWithDriver = rentalType === 'with_driver';
  const effectivePrice = isWithDriver ? car.price_per_day + 500 : car.price_per_day;

  return (
    <div className="group bg-white rounded-3xl border border-slate-200 hover:border-[#D71920] hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col font-montserrat">
      <div className="relative h-48 sm:h-52 w-full bg-slate-100 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex items-center gap-1.5">
          ...
          <span className="px-2 py-0.5 rounded-full bg-[#D71920]/90 backdrop-blur text-white text-[10px] font-bold flex items-center gap-1 shadow-sm">
        ...
        <div className="absolute bottom-3 left-3 flex items-center gap-1.5">
          ...
          {isWithDriver ? (
            <span className="px-2 py-1 rounded-lg bg-[#D71920] backdrop-blur text-white text-[10px] font-black shadow-sm flex items-center gap-1">
              👨‍✈️ Driver Included
            </span>
          ) : (
        ...
        {car.delivery_available && (
          <div className="absolute top-3 right-3">
            <span className="px-2.5 py-1 rounded-full bg-slate-950/90 backdrop-blur text-red-400 text-[10px] font-bold flex items-center gap-1 shadow-sm border border-red-500/30">
              <Truck className="w-3 h-3 text-[#D71920]" />
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        <div>
          ...
          <h3 className="font-extrabold text-lg text-slate-950 group-hover:text-[#D71920] transition tracking-tight">
          ...
          <div className="grid grid-cols-3 gap-2 pt-3 text-xs text-slate-700">
            <div className="flex items-center gap-1.5 bg-slate-50 p-2 rounded-xl border border-slate-100">
              <Fuel className="w-3.5 h-3.5 text-[#D71920]" />
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 p-2 rounded-xl border border-slate-100">
              <Gauge className="w-3.5 h-3.5 text-[#D71920]" />
        ...
        <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
          <div>
            <div className="flex items-baseline gap-1">
              <span className="text-xl font-black text-slate-950">{formatCurrency(effectivePrice)}</span>
              <span className="text-xs text-slate-500 font-medium">/ day</span>
            </div>
            <div className="text-[11px] text-[#D71920] font-bold">
              {isWithDriver ? 'Includes Car + ₹500 Driver' : `Deposit: ${formatCurrency(car.security_deposit)} (Refundable)`}
            </div>
          </div>

          <Link
            href={`/cars/${car.slug}?rentalType=${rentalType}`}
            className="px-4 py-2.5 rounded-xl bg-slate-950 group-hover:bg-[#D71920] text-white text-xs font-bold transition flex items-center gap-1.5 shadow-sm"
          >
            <span>{isWithDriver ? 'Book With Driver' : 'Book Self-Drive'}</span>
